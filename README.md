...
输出的五个字段中使用空格分隔，行末不要有多余的空格。两个 GPA 都需要**精确地四舍五入**到两位小数，如果不足两位请补充零。此外，为方便比较结果，请根据**每个学期在输入文件中第一次出现的顺序**进行输出。
...
我们提供了一个样例 `examples/gpa.in` 和对应结果 `examples/gpa.out`。你也可以运行 `grader.py` 在样例上对你的代码进行评测。你的输出将与标准答案进行全文比较。
