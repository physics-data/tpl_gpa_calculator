...
关于等级到绩点的映射可参见附录，我们保证所有的等级字段都在其中出现。输出的三个字段中使用空格分隔，行末不要有多余的空格。两个 GPA 都需要四舍五入到两位小数，如果不足请补充零。此外，为方便比较，请根据**学期在输入文件中第一次出现的顺序**进行输出。
...
3. 在最终评测时，将会使用规模更大的随机生成的数据，请注意程序的普适性、鲁棒性与效率。
...
此外，还有以下等级不计入 GPA 的计算：`EX`（免修）, `W`（退课）, `I`（缓考）, `P`（通过）。
